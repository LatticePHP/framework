...
During replay, `executeActivity()` returns the previously recorded result. When replay catches up to the end of history, the engine switches to live execution and records new events.
...
`CompensationScope::run()` executes the action; if it succeeds, the compensation is registered. Calling `$scope->compensate()` runs all registered compensations in **reverse order**.
