...
The underlying implementations are `BcryptHasher` and `ArgonHasher` from `Lattice\Auth\Hashing`.

---

## Encryption

The `Encrypter` class (`Lattice\Auth\Encryption\Encrypter`) provides AES-256-GCM authenticated encryption.

```php
use Lattice\Auth\Encryption\Encrypter;

// Generate a key
$key = Encrypter::generateKey('aes-256-gcm'); // Returns hex string
...
All user input is validated through typed DTOs with validation attributes. This prevents injection at the framework level.
...
DTOs are validated before the controller method runs. Invalid input returns 422 with structured errors. This eliminates an entire class of vulnerabilities by ensuring only valid, typed data reaches business logic.

---

## SQL Injection Prevention

LatticePHP uses Illuminate's Eloquent ORM and Query Builder, which parameterize all queries by default. Raw queries should use bindings:
...
$results = DB::select('SELECT * FROM orders WHERE user_id = ?', [$userId]);
...
LatticePHP provides built-in multi-tenancy through workspaces and tenants.
...
`TenantContext` and `WorkspaceContext` provide the current tenant/workspace ID throughout the request lifecycle.
