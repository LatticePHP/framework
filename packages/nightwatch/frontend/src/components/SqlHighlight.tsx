...
function highlightSql(sql: string): React.ReactNode[] {
  ...
  while ((match = pattern.exec(sql)) !== null) {
    ...
    if (match[1]) {
      // SQL keyword
      parts.push(
        <span key={match.index} className="sql-keyword">
          {text}
        </span>,
      );
    } else if (match[2]) {
      // String literal
      parts.push(
        <span key={match.index} className="sql-string">
          {text}
        </span>,
      );
    } else if (match[3]) {
      // Number
      parts.push(
        <span key={match.index} className="sql-number">
          {text}
        </span>,
      );
    } else if (match[4]) {
      // Comment
      parts.push(
        <span key={match.index} className="sql-comment">
          {text}
        </span>,
      );
    }
...
export default function SqlHighlight({ sql, truncate, className }: SqlHighlightProps) {
  const displaySql = truncate && sql.length > truncate ? sql.slice(0, truncate) + '...' : sql;

  return (
    <code
      className={`font-mono text-xs whitespace-pre-wrap break-all ${className ?? ''}`}
    >
