...
import { useMetricsOverview } from '@/api/metrics';
...
export default function OverviewPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-64">
        <Spinner label="Loading metrics..." size="lg" />
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-center py-12 text-danger">
        <p className="text-lg font-semibold mb-2">Failed to load metrics</p>
  ...
  const formatMetricValue = (metric: { value: number; unit?: string }): string => {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          color={data.avg_response_time.value > 500 ? 'danger' : 'success'}
        />
        ...
        <MetricCard
          ...
          color={data.p99_response_time.value > 1000 ? 'danger' : 'warning'}
          onClick={() => navigate('/prod/slow-requests')}
        />
        ...
        <MetricCard
          ...
          color={data.error_rate.value > 5 ? 'danger' : data.error_rate.value > 1 ? 'warning' : 'success'}
          onClick={() => navigate('/prod/exceptions')}
        />
        ...
        <MetricCard
          ...
          color={data.slow_queries_count.value > 10 ? 'danger' : 'warning'}
          onClick={() => navigate('/prod/slow-queries')}
        />
        ...
        <MetricCard
          ...
          color={data.cache_hit_ratio.value > 90 ? 'success' : data.cache_hit_ratio.value > 70 ? 'warning' : 'danger'}
        />
        ...
        {data.cpu_usage && (
          <MetricCard
            ...
            color={data.cpu_usage.value > 80 ? 'danger' : data.cpu_usage.value > 60 ? 'warning' : 'success'}
          />
        )}
        ...
        {data.memory_usage && (
          <MetricCard
            ...
            color={data.memory_usage.value > 85 ? 'danger' : data.memory_usage.value > 70 ? 'warning' : 'success'}
          />
        )}
        ...
        {data.disk_usage && (
          <MetricCard
            ...
            color={data.disk_usage.value > 90 ? 'danger' : data.disk_usage.value > 75 ? 'warning' : 'success'}
          />
        )}
      </div>
    </div>
